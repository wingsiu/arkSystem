...
//by alpha    
 function clone(children, controller) {
    var tmp = clonedeep(children);             
    var pass = (!children.needsLogin() || (controller.isLoggedIn() && controller.isAllowed(children)));
//    console.log("needsLogin:aa", children.needsLogin());
//    console.log("isLoggedin:aa", controller.isLoggedIn());
//    console.log("Allow:aa",controller.isAllowed(children));
//    console.log("children:aa",children);
...
//by alpha
        if (context.fn ==="index.ejs") {
            var controller = context.page.getController(context);
/*            let menu = 
                    context.page.top.children.map(children => {
            return clone(children, controller);            
                    });
            menu = menu.filter(item=>item.item!="");*/
            let topMenu = context.page.root.children.map(children => {             
                 return clone(children, controller);
            });
            topMenu = topMenu.filter(item=>item.item!="");
...
//            console.log("contents", context.session);
                  context.res.set("Access-Control-Allow-Origin", "*");
                    context.res.json({topMenu:topMenu,
                    session:context.session});
                    
/*                    context.res.json({sideMenu:menu, topMenu:topMenu, contents:contents,
                    session:context.session});*/
