...
//by alpha    
 function clone(children) {
    var tmp = clonedeep(children);             
    tmp.parent = tmp.item.parent.id;
    tmp.top  = tmp.top.item.id;
    tmp.root = tmp.root.item.id;
    tmp.content = tmp.content.map(c=>{
        var temp = clonedeep(c);
        temp.page = temp.page.item.id;
        if (typeof(temp.atom) != "undefined") 
                delete temp.atom.app           
        return temp; 
     });
    tmp.children = tmp.children.map(c=>{
        var temp = clonedeep(c);
        temp.parent = temp.parent.item.id;
        temp.top  = temp.top.item.id;
        temp.root = temp.root.item.id;
        temp.content = temp.content.map(d=>{
            var tempContent = clonedeep(d);
            tempContent.page = tempContent.page.item.id;
            if (typeof(tempContent.atom) != "undefined") 
                delete tempContent.atom.app           
            return tempContent; 
        });
        return temp;
     });
...
//by alpha
        if (context.fn ==="index.ejs") {
 //                   if (context.page.parent!==-1) {
            let menu = 
                    context.page.parent.children.map(children => {
            return clone(children);            
                    });
                    
            let topMenu = context.page.root.children.map(children => {             
                 return clone(children);
            });
//                console.log("TopMenu: children", topMenu[0].children);
  //              console.log("TopMenu: content", topMenu[0].content);

 //                 console.log("content:",context.page);
            let contents = context.page.root.content.map(content => {
         //       return clone(content);
                let temp =clonedeep(content);
                temp.page= temp.page.item.id;
                return temp;
                
                     //  return clone(content);
      //                     return {"id": content.id,"item": content.item, "content":content.data}                  
            });
            console.log("contents", contents);
                  context.res.set("Access-Control-Allow-Origin", "*");
//                   let tmp = Object.assign(context.page.parent.children);
                    //const constObj = Object.assign(context.page);
  //                  for (var key in tmp) {
    //                    delete tmp[key].parent;
      //                  delete tmp[key].top;
        //                delete tmp[key].root;
          //              delete tmp[key].content;
            //            tmp[key].item.parent = tmp[key].item.parent.id 
//                        console.log("Content: ", tmp[key].content)

 //                   }
            
 //                   console.log("Application.send response -> menu", topMenu);
                   context.res.json({sideMenu:menu, topMenu:topMenu, content:contents});
